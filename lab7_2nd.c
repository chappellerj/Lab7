...
// Date: 05/23/2023
// Title: Lab 7 Step 3
// Description: C program that uses Second Chance for page replacement
...
int main(int argc, char *argv[]){
    ...
    while (fgets(pageCache, 100, stdin)){
        ...
        if (foundInCache == false){
            for (i = 0; i < cacheSize; i++) {
                if (cache[(placeInArray + i) % cacheSize].flag == 1) {
                    cache[(placeInArray + i) % cacheSize].flag = 0;
                }
                else {
                    placeInArray = (placeInArray + i) % cacheSize;
                    break;
                }
            }
